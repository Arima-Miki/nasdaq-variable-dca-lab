# Experiment Specification — Baseline **v2**

**Status: APPROVED AND PRESERVED — NOT YET EFFECTIVE. NOT CONTROLLING.**

**Baseline version: v2**

**Predecessor: Baseline v1** — `docs/experiment_spec.md`, blob
`3a18862bea98f4134b28e3a5cf160ba9673fc094`, tag `phase0-baseline-v1` —
**unchanged, unamended, and immutable.**

**Effective date: NOT YET SET.** See [Version provenance](#version-provenance).

**Phase: 0 — Baseline Specification**

**Revision: incorporates Owner Decisions OD-01 through OD-14 (all APPROVED),
unchanged from Baseline v1. Baseline v2 changes one provision — v1 §18.3 — and
adds [Section 18.4](#184-simulation-trial-lane).**

### Version provenance

This is an **explicitly versioned successor Baseline**, created under the
change-control mechanism Baseline v1 itself provides: `phase0_baseline_decisions.md`
("Change control after the Freeze") requires a change to *"any other Baseline
rule"* to be made by explicitly creating a **new Baseline version**;
[Section 18.2](#182-effect-of-the-freeze) requires a *"new, explicitly versioned
Baseline created under research governance"* rather than a silent edit; and
[Section 18.3](#183-relationship-to-phase-1-and-phase-2) reserves *"any later
revision"* to the Owner alone.

**Form: FULL RESTATEMENT.** This document is the complete controlling Baseline
specification once activated. A reader does **not** need to apply any patch to
Baseline v1 mentally.

**Approved normative change surface — exactly two items:**

| # | Change |
| - | ------ |
| **1** | **[Section 18.3](#183-relationship-to-phase-1-and-phase-2) is given successor treatment**, separating *formal-study* code from *Simulation Trial* code |
| **2** | **[Section 18.4](#184-simulation-trial-lane) is added**, defining the Simulation Trial governance needed to support that change |

**Every other provision — Sections 0 through 18.2, 19 and 20 — is restated from
Baseline v1 unchanged.** Any difference outside the change surface above is a
**defect**, not an amendment, and MUST stop preservation.

**No unrelated methodology redesign is authorized**, and none is present. No
strategy rule, threshold, sizing parameter, annual budget, cash assumption,
escalation behavior, or Reference High definition changes. OD-01 through OD-14
are unchanged. The Primary Proxy qualification criteria and their freeze
boundary are unchanged.

**The Freeze statement below is retained verbatim from Baseline v1**, because it
records the Phase-0 Freeze as a historical fact that Baseline v2 does not undo.
Baseline v2's own status is carried in this header, not in that statement.

**Activation.** Drafting is not approval; approval is not activation; commit is
not activation; tag creation is not activation. Baseline v2 becomes controlling
only upon a **separate explicit Owner activation statement** fixing its effective
date. **Until then, Baseline v1 — including its Section 18.3 — remains the sole
...
**Formal execution** means code or execution whose output may become, or may be
cited as, formal Baseline evidence, Phase-2 evidence, final strategy evidence, or
any Primary-Proxy-dependent formal study output.

Until those gates are cleared:

- No **formal** backtest code is written.
- No **formal** methodology code is written.
- No **formal** data loaders are written.
- No Baseline results exist, and none may be cited.

`[BASELINE v2 — CHANGED FROM v1]` **Simulation Trial** code
([Section 18.4](#184-simulation-trial-lane)) MAY be developed and executed before
those gates are cleared, subject to **all** of the following:

- it is **structurally barred** from producing formal Baseline evidence
  ([Section 18.4.3](#1843-formal-result-prohibition));
- it is developed and executed **only** under a separate, explicit Owner
  authorization — this specification creates the possibility, never the
  permission;
- it satisfies every constraint in [Section 18.4](#184-simulation-trial-lane);
- **Phase 2 does not begin**, and its entry conditions are unchanged.

Simulation Trial code is **not** Phase-2 code, **not** formal backtest code,
**not** formal methodology code, and **not** a formal data loader. Its existence
discharges no Phase-1 item and satisfies no gate.

Neither this Freeze nor any later revision may be applied by anyone other than
the Owner.

### 18.4 Simulation Trial lane

`[BASELINE v2 — NEW]`

The **Simulation Trial** is a bounded experimental lane whose outputs are
**engineering evidence only**.

> **The Simulation Trial is NOT Phase 2.** Phase 2 remains **BLOCKED** and its
> entry conditions are unchanged. Its existence does not clear a qualification
> gate, approve a Primary Proxy, create formal Baseline evidence, or create final
> strategy evidence.

#### 18.4.1 Two modes — neither self-authorizing

| Mode | Inputs | Purpose |
| ---- | ------ | ------- |
| **Simulation Trial — Mode E** (Engine Validation) | **Synthetic or constructed data only** | Determinism; ATH tracking; drawdown mechanics; trigger mechanics; purchase execution; monthly state transitions; annual-budget accounting; cash accounting; portfolio accounting; invariants; reporting correctness; replay determinism |
| **Simulation Trial — Mode P** (Provisional Economic Simulation) | Provisional, not-yet-qualified historical data | Behaviour comparison; estimating the materiality of unresolved qualification uncertainty |

**This specification authorizes NEITHER mode.** Each requires its own separate
Owner Decision and execution plan. A Mode-E authorization does **not** authorize
Mode P.

**Mode P is a framework placeholder only.** It is defined here so that a later
Owner Decision suffices to authorize it, without requiring a further Baseline
version.

**Mode-E prohibitions, absolute:** economic conclusions are **prohibited**, and
synthetic or constructed data MUST NEVER be represented as market evidence.

#### 18.4.2 Formal and non-formal execution

**Formal execution** is defined in
[Section 18.3](#183-relationship-to-phase-1-and-phase-2) and remains gated.

**Simulation Trial execution is structurally NON-FORMAL.** Its outputs cannot
become formal by relabelling, by agreement across runs, by plausibility, or
because its inputs are later approved.

#### 18.4.3 Formal-result prohibition

Simulation Trial output, in either mode, is **NEVER** eligible to establish:

- Primary Proxy approval; P1-2; P1-5; P1-6; O-4; HG-8;
- formal Baseline performance; Phase-2 completion;
- strategy superiority; live-investment suitability.

Nor MAY it resolve any hard gate, satisfy any Phase-1 item, fix any
[Section 19.2](#192-methodology-requirements-before-implementation) methodology
item, or be cited as Baseline evidence.

Mode-P output, if ever authorized, **remains non-formal**.

#### 18.4.4 Promotion barrier

> **An old experimental run is NEVER promoted. It is re-run, or it is not formal.**

Formal use requires a **fresh formal execution** under the then-approved formal
conditions. Before any formal result may exist, **all** of the following MUST
hold: P1-2 resolved and a Primary Proxy approved; the P1-5 date derived; P1-6
selected; a formal Baseline Dataset constructed; an approved formal simulator
version; reproducibility verification; and a fresh formal execution against the
approved dataset.

#### 18.4.5 Experimental rule variants

A Simulation Trial run MAY execute the frozen Baseline strategy rule as its
execution target without the run becoming a formal Baseline result, because
[Section 18.4.3](#1843-formal-result-prohibition) bars the output regardless of
the rule executed.

A run MAY execute a non-Baseline experimental rule variant only if it is labelled
**`EXPERIMENTAL VARIANT — NOT BASELINE`** in the run manifest and in every output,
and the variant is separately recorded under the existing hypothesis-register
discipline. Executing a variant neither adopts it nor makes it part of any
Baseline.

#### 18.4.6 Sensitivity boundary

[Section 16](#16-no-optimization-before-baseline-evidence) is unchanged:
**sensitivity analysis belongs to Phase 4**, and that remains controlling for
formal study sensitivity analysis.

| Class | Governance |
| ----- | ---------- |
| **Formal study sensitivity analysis** — parameter, threshold, sizing or budget variation intended to inform study conclusions | **Phase 4. Unchanged. Not available in the Simulation Trial** |
| **Engineering robustness testing** — determinism, boundary conditions, invariant checks, replay equivalence, degenerate-input handling; no economic claim | Permitted in **Mode E** |
| **Provisional economic sensitivity** — proxy-choice, FX, expense or alignment perturbation intended to estimate materiality | **Mode P only**, requiring its own Owner Decision. Not permitted in Mode E |

This boundary MUST NOT be crossed by relabelling. A test that produces an economic
comparison is not an engineering robustness test, whatever it is called.

#### 18.4.7 Anti-contamination

Simulation Trial results become known. Blinding is neither claimed nor practical.
The controlling distinction is therefore between a **KNOWN BUT NON-NORMATIVE
EXPERIMENTAL RESULT** and **FORMAL QUALIFICATION EVIDENCE**.

Simulation Trial output MUST NOT be used to: select a candidate because it
performs better; derive or redefine P1-5; select P1-6 opportunistically;
reinterpret O-4; weaken HG-8; alter any evidence hierarchy; alter any
search-exhaustion rule; exclude an unfavourable candidate; or change strategy
thresholds outside a separately governed hypothesis process.

**Disclosure rule.** Where a qualification decision is taken after any Simulation
Trial result exists, the deciding artifact MUST state that such results were known
and affirm that they were not used normatively.

#### 18.4.8 Reversibility

The Simulation Trial MUST remain abandonable at any time: the qualification work
continues unaffected; experimental code may be discarded; **no formal evidence may
ever depend on it**; and no frozen strategy result changes.

#### 18.4.9 Run manifest

**No Simulation Trial run may execute without a manifest.** Each manifest MUST
record: run ID; execution mode (E or P); simulator version or commit; strategy-rule
identifier; whether the rule is the Baseline rule or an
`EXPERIMENTAL VARIANT — NOT BASELINE`; dataset ID; dataset class (synthetic,
constructed, replay, or provisional); parameters; date range where applicable;
assumptions; output metrics; the explicit status **`NON-FORMAL — SIMULATION
TRIAL`**; prohibited uses; known limitations; and `baseline_version: v2`.

For Mode E the dataset entry MUST state **synthetic** or **constructed**
explicitly.

**A run without a manifest is void, and its output MUST NOT be used for any
purpose.**
